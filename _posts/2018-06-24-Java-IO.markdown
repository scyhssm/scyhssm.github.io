...
1.File 类可以用于表示文件和目录的信息，但是它不表示文件的内容。递归地输出一个目录下所有文件：
```
...
```
![DecoratorIO](/img/DP-Decorator-java-io.png)
Java I/O 使用了装饰者模式来实现。以 InputStream 为例，InputStream 是抽象组件，FileInputStream 是 InputStream 的子类，属于具体组件，提供了字节流的输入操作。
...
InetAddress，没有公有构造函数，只能通过静态方法来创建实例。
...
![Socket通信](/img/client-server.jpg)￼

Datagram
* DatagramPacket：数据包类
* DatagramSocket：通信类

6.NIO，新的输入/输出 (NIO) 库是在 JDK 1.4 中引入的。NIO 弥补了原来的 I/O 的不足，提供了高速的、面向块的 I/O。
...
* position：当前已经读写的字节数；
* limit：还可以读写的字节数。
状态变量的改变过程，capacity不变。position刚开始为0，如果有数据被从输入通道读入缓冲区，position增加，limit指向capacity的位置，position最多增长到limit的位置。要将缓冲区的数据写到输出通道，要调用flip方法，把position重新移动在初始位置，limit移到position位置。把字节输出到输出通道，最后调用clear清空缓冲区。

NIO实例，可以看到NIO被集成到了I/O中：
```
...
应该注意的是，只有套接字 （TCP）Channel 才能配置为非阻塞，而 FileChannel 不能，为 FileChannel 配置非阻塞也没有意义，服务器客户端模式。
￼
选择器流程：
