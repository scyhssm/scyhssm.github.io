...
## 实现
部分人的yum源下载ipvsadm会失败，最好能够有一个靠谱的yum源。网上找一下yum源的配置，镜像站很多。  
真实机器：  
192.168.158.101  
192.168.158.102  
192.168.158.103  
...
绑定虚拟ip
```
ifconfig eth0:0 192.168.158.200 broadcast 192.168.158.200 netmask 255.255.255.255 up
```
为eth0:0设置一条路由
```
route add -host 192.168.158.200 dev eth0:0
```
启用包转发功能，令系统充当路由
```
